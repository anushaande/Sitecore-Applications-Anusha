Event.observe(document, "dom:loaded", function() {  
  ...
  if (sortList) {
    var isDbView = $("dbv");
    var sortingFormat = /^([0-9a-zA-Z]{32})$/;  // default format for sorting input view items by sitecore ID
    if (isDbView) { sortingFormat = /^([0-9]{4,7})$/; } // format for sorting db entries by person ID
    Sortable.create("sort-list", {scroll: "MainContainer", elements: $$(".sort-item.editable"),  format: sortingFormat, onUpdate: function() {
          scPersistSortOrder();
        }
    });  
  }
