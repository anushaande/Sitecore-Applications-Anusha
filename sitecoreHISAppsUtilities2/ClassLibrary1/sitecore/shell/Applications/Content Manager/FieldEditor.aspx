		...
		<script>
		        ...
		        for (i = 0; i < inp.length; i++) {
		            if (inp[i].value == "OK") {
		                if (p.autoSaveItems != false) {
		                    btn = inp[i];
		                    //p.console.log('btn onclick: ' + jQuery(btn).attr('onclick'));
		                    btn.setAttribute("onclick", "javascript:return handleOkClick()");
		                }
		                break;
		            }
		        }
		    ...
		    function handleOkClick() {
		        ...
		        if (p.Sitecore.PageModes.PageEditor.isModified()) {
		            p.Sitecore.PageModes.PageEditor.postRequest("webedit:save", null, false);
		        }

		        scForm.invoke("contenteditor:save");
		        p.Sitecore.PageModes.PageEditor.postRequest("webedit:saved", null, true);
		    }
		</script>
      </div>
    </div>
