...
var SitecoreMediaUploader = Class.create({
    ...
    // Update by Jihyun Kim on 11/10/2016
    onUploadError: function (event) {
        //console.warn("upload error. file: %s, message: %s", event.id, event.status);
        this.hadErrors = true;
        ...
        row.removeClassName("queued");
        var progress = row.down('.progress');
        var additionalMessage = "<span style='color:red; font-size:12px; padding-left: 12px; float: right;'>The file size exceeds max limit (" + this.settings.uploadImageLimit + ")</span>";
        progress.down('img').remove();
        progress.setStyle({ backgroundImage: 'none' });
        progress.insert({
            bottom: new Element("img", {
                src: '/~/icon/Office/16x16/delete.png.aspx', alt: event.status, title: event.status
            }),
            bottom: additionalMessage
        });
