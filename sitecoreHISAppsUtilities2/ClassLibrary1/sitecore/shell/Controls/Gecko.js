...
scBrowser.prototype.closePopups = function(reason, exclusions) {
	...
	if(typeof window != 'undefined' && window != null){
		wTop = getTopMostWindow(window, 0);
	}
	if(wTop != null){
		if (wTop.popups != null) {
			...
			for(var n = 0; n < wTop.popups.length; n++) {
				if (exclusions && exclusions.indexOf(wTop.popups[n]) >= 0) {
					continue;
				}
				var ctl = $(wTop.popups[n]);      
				if (ctl) {
					ctl.remove();
				}
			}
			wTop.popups = exclusions ? $$(".scPopup") : null;
 ...
 function getTopMostWindow(currentWindow, ct){
	if(currentWindow != null 
		&& typeof currentWindow.parent != 'undefined' 
		&& ct < 10)
	{
		ct++;
		currentWindow = getTopMostWindow(currentWindow.parent, ct);
	}
...
scBrowser.prototype.showModalDialog = function (url, arguments, features, request) {
  ...
  this.insertAdjacentHTML(window.top.document.body, 'afterbegin', '<div id="ModalOverlay" style="position: fixed; left: 0px; top: 0px; width: 100%; height: 100%; z-index: 10002; background-color: rgb(170, 170, 170); filter:alpha(opacity=50); opacity: 0.5;"></div>');
