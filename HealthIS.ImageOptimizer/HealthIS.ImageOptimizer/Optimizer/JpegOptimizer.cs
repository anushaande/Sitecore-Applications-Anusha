...
namespace HealthIS.Validations.Compression.Jpeg
{
    public class JpegOptimizer : IImageOptimizer
    {
        ...
        public IOptimizerResult Optimize()
        {
            ...
            Process process = Process.Start(this.ToolPath, StringExtensions.FormatWith(" -optimize -copy none -progressive -outfile \"{0}\" \"{0}\"", (object)tempFilePath));
            ...
            jpegOptimizerResult.Success = false;
            jpegOptimizerResult.ErrorMessage = StringExtensions.FormatWith("jpegtran took longer than {0} to execute, which we consider a failure.", (object)this.ToolTimeout);
            return (IOptimizerResult)jpegOptimizerResult;
        }
