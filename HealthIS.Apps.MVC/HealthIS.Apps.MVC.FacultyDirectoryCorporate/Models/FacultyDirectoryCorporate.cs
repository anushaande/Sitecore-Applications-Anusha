...
{
    public class FacultyDirectoryCorporate
    {
        ...
        //
        public CorporatePersonInfo SearchHD(string fname, string lname, string personIdFromList)
        {
            ...
            HealthIS.Apps.MVC.Models.CorporatePersonInfo person = new HealthIS.Apps.MVC.Models.CorporatePersonInfo();
            try
            {
            ...
            }catch (Exception ex)
            {
                ...
                //showError = "<h3>Sorry for the inconvenience but we are performing some maintenance at the moment.</h3>";
                errorMessage += ex.InnerException == null ? "" : "<br/>Inner Exception: " + ex.InnerException.Message;
                Sitecore.Diagnostics.Log.Error("Faculty Direcotry Error - 'SearchHD' method: ", errorMessage);
            }
            return person;
        }
        ...
        public string FacultyInfo(string personID)
        {
            ...
            catch (Exception ex)
            {
                errorMessage = "Error: " + ex.Message + "<br/>" + ex.StackTrace;
                errorMessage += ex.InnerException == null ? "" : "<br/>Inner Exception: " + ex.InnerException.Message;
                Sitecore.Diagnostics.Log.Error("Faculty Direcotry Error - 'FacultyInfo' method: ", errorMessage);
            }
        ...
        //
        public CorporatePersonInfo LoadFacultyInfo(NameValueCollection allFaculties, string personId)
        {
            HealthIS.Apps.MVC.Models.CorporatePersonInfo person = new HealthIS.Apps.MVC.Models.CorporatePersonInfo();
            try
            {
                ...
                if (!personId.ToLower().Contains("temp_") && allFaculties.Count > 0)
                {
                    ...
                    {
                        CorporatePersonInfo searchedData = SearchHD(person.FIRST_NAME, person.LAST_NAME, personId);
